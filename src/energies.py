...
# ----------------------------------------------------------
def _element_curl_A(
    A_nodes: jnp.ndarray, conn: jnp.ndarray, grad_phi: jnp.ndarray
) -> jnp.ndarray:
    """
...
def brown_energy_and_grad_from_scalar_potential(
    ...
    grad_u_term1 = (
        -MU0 * jnp.einsum("eak,ebk,eb->ea", grad_phi, grad_phi, u_e) * Ve[:, None]
    )  # (E,4)
    ...
    contrib_m_e = (MU0 * Ms_e * Ve / 4.0)[:, None, None] * gradU_e[
        :, None, :
    ]  # (E,4,3)
